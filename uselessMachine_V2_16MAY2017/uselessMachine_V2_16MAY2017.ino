    ...
    if(uitoggleCount == THE_ANSWER_TO_LIFE) 
    {
      if(iYourScore > SCORE_THRESHOLD)
      {
        ...
        if(checkLockStatus())
        {
          ...
          {
            displayLCD(F("Congratulations"),F("Louie!"), 60,5000);       
            displayLCD(F("You Cracked"),F("the code..."), 0,6000);    
...
void displaySensors(void)
{
  ...
  if(bcheckSwitchState) displayPerseverance();
...
void dispatchBehaviors(void)
{
  ...
  // modulate behavior based on perseverence.  the longer the player plays, the more special behaviors are expressed
