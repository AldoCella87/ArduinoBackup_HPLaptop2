...
#define TEST_MODE
  ...
  #define PERSEVERANCE_THRESHOLD  0                 // reading required to unlock
  #define LIGHT_THRESHOLD 0                         // reading required to unlock
  #define MAG_THRESHOLD 0                           // reading required to unlock
  #define DISTANCE_THRESHOLD 0                      // reading required to unlock
...
void loop() {
  ...
  else if(itoggleCount == 5) 
  {
    ...
    *********************************************************/
    int imagReading = 0;
    iSwitchState = checkSwitchState();  
    if(iSwitchState == 1 && (random(1,101) <= MAGNETO_PROB)) imagReading = imagneto();
...
int checkLockStatus()
{
    ...
    lcd.setCursor (0,1);        
    if((iPerseverance > PERSEVERANCE_THRESHOLD) || (bPerseverance == 1))
    {
    ...
    lcd.clear();          
    lcd.home (); 
    lcd.print("Mag orientation:"); 
    lcd.setCursor (0,1);  
          
    if(((iMagOrient = readMagSensor()) > 0) || (bMagStatus == 1))
    {
      bMagStatus = 1;            
    ...
    lcd.clear();          
    lcd.home (); 
    lcd.print("Light sense:   "); 
    lcd.setCursor (0,1);    
    // read light sensor here
    iLightSense = 9999;   //test value here
    if((iLightSense > LIGHT_THRESHOLD) || (bLightStatus == 1))
    {
      bLightStatus = 1;           // once unlocked remain unlocked
    ...
    lcd.clear();          
    lcd.home (); 
    lcd.print("Distance sense: "); 
    lcd.setCursor (0,1);   
    // get distance sensor value here...
    iDistance = 9999;  // test value here 
    if((iDistance > DISTANCE_THRESHOLD) || (bDistanceStatus == 1))
    {
      bDistanceStatus = 1;           // once unlocked remain unlocked
      lcd.print(iDistance); 
      lcd.print(" UNLOCKED"); 
      //warble(100);   
    }
