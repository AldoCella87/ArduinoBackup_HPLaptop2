...
#define  TEST_MODE
  ...
  #define PERSEVERANCE_THRESHOLD  55                 // miinimum number of rounds required to obtain perseverance unlock  
  #define SCORE_THRESHOLD  130                        // minimum score required to obtain score unlock  
...
void loop() {
  ...
  { 
      ...
      if(iYourScore > SCORE_THRESHOLD)
      {
        ...
        if(checkLockStatus() == LOCK_STATUS_UNLOCKED)
        {
          if(1) //  add password check, score check or other gate here if desired...
          {
            ...
            displayLCD(F("AQEM - 8EDQHM"),F("4PVWC"), 0,6000);   // just short enouth to cause consternation (next time have a pencil and paper handy)
    ...
    // done with multi toggle processing, proceed with processing based on trial count
