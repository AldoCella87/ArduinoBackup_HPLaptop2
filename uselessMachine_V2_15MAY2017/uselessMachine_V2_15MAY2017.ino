...
#define  EST_MODE
  ...
  #define PERSEVERANCE_THRESHOLD      190         // miinimum number of rounds required to obtain perseverance unlock  
  #define SCORE_THRESHOLD             350         // minimum score required to obtain score unlock  
...
void loop() {
  ...
  { 
      ...
      if(iYourScore > SCORE_THRESHOLD)
      {
        ...
        if(checkLockStatus())
        {
          if(1) //  add password check, score check or other gate here if desired...
          {
            ...
            Serial.println("XXXX - XXXXXX - XXXXX");  
            // reset scores
            swReset();
          }
          else
    ...
    // Check 4 - create a hint that the machine is aware of multiple toggles
...
void dispatchBehaviors(void)
{
  ...
  // modulate behavior based on perseverence.  the longer the player plays, the more special behaviors become evident
  ...
  //switch(myrandom(1,(analogRead(LIGHT_SENSE_PIN)+ NUM_CASES + 1))) // have probability of special behavior be related to light level, ensure that minimum of 'n' behaviors available                                                 
  {
   ...
   case 2:
    ...
    case 15:  
      if(itrialCount > DISPLAY_READINGS_MIN + 60) displayPerseverance();
      break;
