...
// when in operation navigate to the following addresses for administration
// 172.0.0.1/index - this is what the user sees
// 172.0.0.1/post  -  user will be redirected here after posting password
// 172.0.0.1/pass  -  show stored passwords
// 172.0.0.1/ssid  -  config page to change the displayed SSID
//  for educational use only  - do not deploy this 
// change to be a simple public bulletin board page

// heavily modified to publish PW digital memorial
...
String sMessage =   "Hello Friends.  Have a beer and a slice of pizza in memory of good old Papa Woody (1959 - 2020).  Remember to love each other and to treat each "
                    ...
                    "... disconnect from this WiFi access point to continue";


// Default SSID name
//const char* SSID_NAME = "Free WiFi";
//const char* SSID_NAME = "Your Lucky Number";
const char* SSID_NAME = "Papa Woody Memorial";
...
#define BODY "Select your lucky number below then submit it to the universe.  Disconnect from this WiFi to continue.  Thanks for your input!  Namaste"
#define POST_TITLE "Updating..."
//#define POST_BODY "Update in process.</br>Thank you."
#define POST_BODY "Sending your selection to the universe... Have a great day!"
...
String index() {
  ...
  //return header(TITLE) + "<div>" + BODY + "</ol></div><div><form action=/post method=post><label>Data:</label>"+
  return header(TITLE) + "<div>" + sMessage + "</ol></div><div><form action=/post method=post><label></label>";  
   // "<input type=password name=m></input><input type=submit value=Start></form>" + footer();
   // "<input type=range name=m></input><input type=submit value=Submit></form>" + footer();
