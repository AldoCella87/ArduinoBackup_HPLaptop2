 ...
 * 3/6/17 - added easter egg - link to gift card
 ***********************************************************************************************/
...
byte ioffDetected = 0;           // used to determine multiple switch toggles
...
void loop() {
  ...
  { 
  iprevSwitchState = 1;            
  itoggleCount++;                                       // we have detected the first switch on state
  ...
  {
    checkSwitchState();
    if(ioffDetected == 1 and iSwitchState == 1)
    {
      ioffDetected = 0;
      itoggleCount++; 
      linow = millis();                                   // user is toggling switch, continue to wait for more input     
    }
    if(iSwitchState == 0) ioffDetected = 1;
  }
...
void checkSwitchState()
{
  ...
  while(iSwitchState != iPrevSwitchState )
  {
...
void flicker(int iflickerCount)
{
  ...
  while(i++ < iflickerCount && iSwitchState == 1)
  {
    ...
    delay(random(23,871));
    checkSwitchState();                                 // user may have changed switch setting, re-read and act accordingly 
    lcd.setCursor (8,1);                    
    lcd.print(inextMoveNumber++);
