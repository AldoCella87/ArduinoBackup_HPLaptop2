// 6/12/21 - based on CableTest20210603_V4 test using Arduino Nano
/****************************************************************
 * Nano specific comments:
 * Set tools processor:  "Atmega328P (Old Bootloader)"
 * 
 * Serial Clock and Serial Data are on Analog 5 and Analog 4 respecitvely (some sources state that wire.h is required 
 * although board works without it
 * 
 * 
 * 
 * 
...
#define SWITCH_PIN              A0  // using analog pin as input.  Read optoisolator output transistor with 1K pullup resistor to arduino 5V.  
                                    // tests show that with switch open value is around 1000 and with switch closed value is around 800.  This is 
                                    // when feeding the optoisolator LED using the power on switch and a 1K resistor in series wiht the isolator LED.
...
long  lOnDuration = 15000;           // duration in miliseconds before moving to next step
 ...
 *   
 *   Mosfet:
 *      Gate:  N.O. Pushbutton through 10K resistor to Vdd (battery +) to activate - done
 *      Gate:  330K resistor in parallel with 2.2uf capacitor to Vss (battery negative (NOT arduino gnd) - done
 *      Gate:  1.1K from emitter of H11 optoisolator - done
 *      Drain: to Arduino ground (to switch negative side of power to negative side of battery) - done
 *      Source: to negative side of battery (Vss)
 *   Battery: 
 *      Positive side of battery: to Arduino Vin pin and to one side of N.O. pushbutton switch - done
 *   H11 optoisolator 1:  Turns unit on
 ...
 *      Anode: to Arduino digital output ISOLATOR_ANODE - done
 *      Cathode: to Arduino digital output ISOLATOR_CATH through a 220 ohm resistor - done
 *      Collector: to Vdd (battery +) - done
 *      Emitter: to gate of MOSFET via 1.1K resistor - done
 *   H11 optoisolator 2:  enables the use of the switch to turn on the unit also serve as a digital input to the unit
 *      Feed anode through 1K resistor from switch
 *      
...
void setup() {
  ...
  pinMode(SWITCH_PIN, INPUT);                       // reads the power-on switch, this allows the power-on switch to also serve as an input
  ...
  DisplayWelcomeMsg();
  while(analogRead(SWITCH_PIN) > 900 && millis() < lStartTime + lOnDuration){};
  lStartTime = millis();
  delay(1000);
}
...
void loop() {
  ...
  // initializw timer for next step
  lStartTime = millis();
  // display results
  DisplaySummary();
  while(analogRead(SWITCH_PIN) > 900 && millis() < lStartTime + lOnDuration){}; 
...
void DisplayWelcomeMsg(void)
{
    ...
    lcd.print(iVersion);
    lcd.setCursor (0,2);  
    lcd.print(F("Connect cable.  Test"));
    lcd.setCursor (0,3);  
    lcd.print(F("will begin shortly"));
    SonalertControl(SOUND_CLICK);  
    while(analogRead(SWITCH_PIN) < 900){};  //trap condition where user has finger on button following power on. This prevents
                                            // bypassing welcome message
    lStartTime = millis();
    delay(400);
    while(analogRead(SWITCH_PIN) > 900 && millis() < lStartTime + lOnDuration){};  // wait till button press or time to move on
...
{
  int iTestDuration = TEST_DURATION;            
