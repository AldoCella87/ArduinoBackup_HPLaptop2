...
int ConductTest(unsigned long ulStartTime)  
{
  ...
  while (millis() < ulStartTime + iTestDuration)
  {
    ...
    // Section D - check for cross connected pins (e.g. pin 1 on MIC side connected to pin 2 of MXR side
    // this test is only relevant if continuity test fails
    // NOTE - this is not implemented.  rely on continuity test for now.  continuity test will fail if pins are cross connected.
