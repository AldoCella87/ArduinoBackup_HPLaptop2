...
/* Ping))) Sensor
  ...
  * SIG connection of the PING))) attached to digital pin 7
   ...
   by Tom Igoe
   Modified 01 DEC 2017 by AE - added RYG Indicators, saved as ParkingControl
   Modified 02 DEC 2017 by AE - added LCD display, saved as ParkingControl1
   // Changes are intended to be as simple as possible in order to illustrate the 
   // fundamentals. They may be refined as desired. 
...
#include <LiquidCrystal_I2C.h>  // custom third party library containing functions for LCD
...
void loop() {
  ...
  // distance greater than 16 - signal yellow = proceed with caution
  {
      ...
      lcd.write("Proceed with");      // send message
      lcd.setCursor(0,1);             // set cursor to character 0 of line 1 
      lcd.write("Caution");           // send message
  ...
  // distance less than 10 - back up before you hit something!
  {
      ...
      // LCD display output
      lcd.setCursor(0,0);            // set cursor to character 0 of line 0 
      lcd.write("Back up!");         // send message
  ...
  // Juuuust right - stop anywhere within this zone
  {
      ...
      Serial.print(" *** GREEN");     // display message to serial monitor   
       // LCD display output
      lcd.setCursor(0,0);             // set cursor to character 0 of line 0 
      lcd.write("Perfect!");          // send message
  }
