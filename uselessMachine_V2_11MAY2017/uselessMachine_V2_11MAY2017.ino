...
bool bTimeFlag1 = 0;            // enables special behavior based on time game has been in play
...
#define  TEST_MODE
  ...
  #define TIME_THRESHOLD 5                          // minimum minutes of play required to unlock  
  ...
  #define FLAP_PROB         1
  #define DISPLAY_READINGS_MIN  1                  // minimun number of trials required before sensor readings are displayed
  ...
  #define SPEAK_TO_ME_PROB  4                       // simulated voice input processing
  #define SPEAK_TO_ME_MIN   25                      // minimum number of trials before speak to me is an option
...
void loop() {
  ...
  { 
      ...
      if(iYourScore > SCORE_THRESHOLD)
      {
        ...
        if(checkLockStatus() == LOCK_STATUS_UNLOCKED)
        {
          if(1) //  add password check, score check or other gate here if desired...
          {
            ...
            displayLCD(F("Game OVER"),F(""), 0,6000);    

            // reset scores
            iMyScore = iYourScore = 0;
            
            //displayLCD(F("Secret Password"),F("Follows:"), 60,5000);       
            //displayLCD(F("Swordfish"),F(""), 0,6000);   // just short enouth to cause consternation (next time have a pencil and paper handy)
          }
          else
          { 
    ...
    else if(itoggleCount == 2) flap(itoggleCount);
...
// ensure all math with millis is unsigned
     ...
     * 
     ********************************************************/
    if((millis() / 60000)   > TIME_THRESHOLD && bTimeFlag1 == 0)
    {
      // set some special behavior here
      bTimeFlag1 = 1;     // do this behavior only once
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,101) <= FAKEOUT_PROB)) fakeout();
    ...
    *********************************************************/
    ...
    if(bSwitchState == 1 && (myrandom(1,101) <= TICK_PROB)) tick(1,(analogRead(LIGHT_SENSE_PIN) / 10),90);
    //void tick(int iduration, int icount, int idutycycle)
 

    /*********************************************************
    * 
    * sound feedback based on mag sensor
    * 
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    //if(bSwitchState == 1 && (myrandom(1,101) <= SONIC_FEEDBACK_PROB)) soundFeedback(5);        //sound feedback based on mag sensor reading (duration in secs)
    if(bSwitchState == 1 && (myrandom(1,101) <= SONIC_FEEDBACK_PROB)) soundFeedback(analogRead(LIGHT_SENSE_PIN)/8);        //sound feedback based on mag sensor reading (duration based on light reading)
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,101) <= NOISE_PROB)  && (itrialCount > DISPLAY_READINGS_MIN /*+ 40*/ ) ) iDisplayNoiseReading();
...
void iDisplayNoiseReading(void)
{
  ...
  linow = millis();
  while((millis() < linow + 96000) && (bSwitchState == 1))
  {
    lcd.setCursor (0,1); 
    lcd.print(F("            "));  
    lcd.setCursor (0,1); 
...
{
  unsigned long iend = millis() + (iduration * 1000);   
  while((millis() < iend) && (bSwitchState == 1))
  {
    //display ms remaining
    lcd.clear();                              
    lcd.home();  
    lcd.print(iend - millis());
    // produce sound
    digitalWrite(ALERT_PIN, 1);         // sonic alert on
    delay(1);
    digitalWrite(ALERT_PIN, 0);         // sonic alert off
...
{    
     ...
     * 
     *************************************/      
    unsigned long uinow = millis() / 60000;
    lcd.clear();          
    lcd.home (); 
    lcd.print(F("Time:")); 
    lcd.setCursor (0,1);        
    if(uinow > TIME_THRESHOLD)
    {
      bTime = 1;                 // once unlocked remain unlocked
      lcd.print(uinow);       
      lcd.print(F(" = UNLOCKED")); 
      //warble(100);
    }
    ...
    linow = millis();
    while((millis() < linow + LOCK_DISPLAY_TIME) && (bSwitchState == 1))
    {
      lcd.setCursor (0,1);  
    ...
    *******************************************************************/
    ...
    if(bPerseverance && bMagStatus && bLightStatus && bDistanceStatus && bScore && bTime) 
    {
...
//test http://playground.arduino.cc/Code/AvailableMemory 
int freeRam () {
