...
#define  TEST_MODE
  ...
  #define TIME_THRESHOLD 1                          // minimum minutes of play required to unlock  
  ...
  #define TICK_PROB         100
  #define SONIC_FEEDBACK_PROB   00
  #define LIGHT_PROB        0
  #define ACCEL_PROB        0
...
// ensure all math with millis is unsigned
     ...
     * 
     ********************************************************/
    if((millis() / (unsigned int) 60000)   > TIME_THRESHOLD && bTimeFlag1 == 0)
    {
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState(); 
    if(bSwitchState == 1 && (myrandom(1,101) <= SONIC_FEEDBACK_PROB)) soundFeedback(analogRead(LIGHT_SENSE_PIN)/SOUND_FEEDBACK_DURATION);        //sound feedback based on mag sensor reading (duration based on light reading)
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,101) <= ACCEL_PROB) && (itrialCount > DISPLAY_READINGS_MIN + 0)) iDisplayAccelReading();
...
void iDisplayNoiseReading(void)
{
  ...
  uliNow = millis();
  while((millis() < uliNow + (unsigned int) 96000) && (bSwitchState == 1))
  {
    lcd.setCursor (0,1); 
    lcd.print(F("            "));  
    lcd.setCursor (0,1); 
