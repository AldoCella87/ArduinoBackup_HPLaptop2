...
#define ONE_HOUR_MS           3600000   //millis per hour
...
#define  xEST_MODE
  ...
  #define PERSEVERANCE_THRESHOLD  1                 // minimum number of rounds required to obtain perseverance unlock  
  ...
  #define ORIENTATION_PROB  0
  #define TICK_PROB         0
  #define SONIC_FEEDBACK_PROB   100
  #define LIGHT_PROB        0
  #define ACCEL_PROB        0
  #define DISTANCE_PROB     0
  #define NOISE_PROB        0  
  #define FAKEOUT_PROB      0  
  #define PERSEVERANCE_PROB 0
  ...
  #define SCORE_THRESHOLD  150                      // minimum score required to obtain score unlock  
  ...
  #define TICK_PROB         1
  #define SONIC_FEEDBACK_PROB   1 
  #define LIGHT_PROB        1
  #define ACCEL_PROB        1
  #define DISTANCE_PROB     1
  #define NOISE_PROB        1  
  #define FAKEOUT_PROB      1  
  #define PERSEVERANCE_PROB 1
  #define FLICKER_PROB      1
  #define PEEK_PROB         1
  #define MULTI_PEEK_PROB   1      
  #define LONG_WAIT_PROB    1
  #define SLOW_DEPLOY_PROB  1
  #define SCORE_BUMP_PROB   1
  #define YOU_WIN_PROB      1
  #define GO_AWAY_PROB      1
  #define SPEAK_TO_ME_PROB  1                       // simulated voice input processing
  #define SPEAK_TO_ME_MIN   25                      // minimum number of trials before speak to me is an option
  ...
  #define SOUND_FEEDBACK_DURATION 15                 // used to scale down sensor reading for length of sound display  
  #define MAX_RAND        100                        // used to set maximum range for random behavior, higher number = less frequent special behavior       
#endif
...
void loop() {
  ...
  // TEST - set degree of special behavior based on external input.  the lower the number the more Sori exhibits quirky behavior
  iMaxRandom = analogRead(LIGHT_SENSE_PIN);
      ...
      if(iYourScore > SCORE_THRESHOLD)
      {
        ...
        if(checkLockStatus())
        {
          if(1) //  add password check, score check or other gate here if desired...
          {
            ...
            displayLCD(F("XXXX - XXXXXX"),F("XXXXX"), 0,6000);   // just short enouth to cause consternation (next time have a pencil and paper handy)
...
// ensure all math with millis is unsigned


    /********************************************************
     * 
     * process various behaviors based on number of trials
     * 
     * 
     * 
     ********************************************************/
     ...
     * 
     ********************************************************/
    if((millis() / (unsigned int) 60000)   > TIME_THRESHOLD && bTimeFlag1 == false)
    {
    ...
    //proceed to one or more random behaviors
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= LONG_WAIT_PROB))longWait();


    /*********************************************************
    * 
    * slowly deploy and then retract actuator without
    * turning switch off
    * 
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= SLOW_DEPLOY_PROB)) slowDeploy();

    /*********************************************************
    * 
    * fakeout -  deploy and then retract actuator without
    * turning switch off
    * 
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= FAKEOUT_PROB)) fakeout();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= PEEK_PROB)) peekout(myrandom(600,1200), myrandom(900, 1700));
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    // number of ticks dependent upon light reading
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= TICK_PROB)) tick(1,(analogRead(LIGHT_SENSE_PIN) / 10),90);
    //void tick(int iduration, int icount, int idutycycle)
 

    /*********************************************************
    * 
    * sound feedback based on sensor reading
    * 
    *********************************************************/
    bSwitchState = bcheckSwitchState(); 
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= SONIC_FEEDBACK_PROB)) soundFeedback(analogRead(MICROPHONE_PIN)/SOUND_FEEDBACK_DURATION);        //sound feedback based on mag sensor reading (duration based on light reading)
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= FLICKER_PROB)) flicker(myrandom(3,27));
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= MULTI_PEEK_PROB))
    {   
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= SPEAK_TO_ME_PROB) && (itrialCount > SPEAK_TO_ME_MIN)) speak();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= ORIENTATION_PROB) && (itrialCount > DISPLAY_READINGS_MIN + 20)) DisplayMagReading();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= LIGHT_PROB) && (itrialCount > DISPLAY_READINGS_MIN + 40)) iDisplayLightReading();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= ACCEL_PROB) && (itrialCount > DISPLAY_READINGS_MIN + 0)) iDisplayAccelReading();

    /*********************************************************
    * 
    * display noise reading
    * 
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= NOISE_PROB)  && (itrialCount > DISPLAY_READINGS_MIN /*+ 40*/ ) ) iDisplayNoiseReading();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= DISTANCE_PROB) && (itrialCount > DISPLAY_READINGS_MIN +60)) iDisplayDistanceReading();

    /*********************************************************
    * 
    * display perseverance reading
    * 
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= PERSEVERANCE_PROB) && (itrialCount > DISPLAY_READINGS_MIN + 80)) iDisplayPerseverance();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,iMaxRandom) <= FLAP_PROB)) flap(myrandom(2,13));
 ...
 * 
 *********************************************************************/
void iDisplayAccelReading(void)
{
  lcd.clear();                              
  lcd.home();  
 ...
 * 
 *********************************************************************/
void iDisplayNoiseReading(void)
{
  lcd.clear();                              
  lcd.home();  
...
void actuatorForward(int iDistance)
{
  lcd.off();
  actServo.attach(ACT_PIN);           
  actServo.write(iDistance);                 // Step 3b - move actuator forward to turn switch off
...
void displayScores()
{
  ...
  // display results
  if((myrandom(1,iMaxRandom) <= YOU_WIN_PROB) && (bYouWon == false) && (itrialCount > 2))
  {      
    // taunt option 1   
  ...
  // taunt 2
  else if((myrandom(1,iMaxRandom) <= I_AM_SORI_PROB) && (bSori == false) && (itrialCount > 4))
  {      
  ...
  if((myrandom(1,iMaxRandom) <= GO_AWAY_PROB) && (itrialCount > 6))
  {                    
...
void swReset() // Restarts program from beginning but does not reset the peripherals and registers
{
  ...
  // this appears to reset program variables (e.g. perseverance) so is useful to reset locks after
  // player wins.
  asm volatile ("  jmp 0");  
} 

void dispatchBehavior(void)
{

  switch(myrandom(1,40)
  {
    case 1:
