  ...
  if(itoggleCount == 5) 
{
    ...
    *********************************************************/
    iSwitchState = checkSwitchState();  
    if(iSwitchState == 1 && (random(1,100) <= SPEAK_TO_ME_PROB)) speak();
...
static int iprogress = 1;
  ...
  switch (iprogress)
  {
    case 1:           
        ...
        lcd.home();  
        // add one or more conditional taunts here...
        lcd.print("speak case 1    ");
        lcd.setCursor (0,1);  
        lcd.print("                ");
        lcd.on();
        iprogress ++;                               // index to next behavior
        break;
