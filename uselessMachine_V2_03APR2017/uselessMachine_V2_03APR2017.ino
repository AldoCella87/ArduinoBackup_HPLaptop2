...
#define  TEST_MODE
  ...
  #define PERSEVERANCE_THRESHOLD  1                 // number of rounds required to obtain perseverance unlock  
  #define LIGHT_THRESHOLD 30                        // reading required to unlock
  ...
  #define PERSEVERANCE_THRESHOLD  91                // number of rounds required to obtain perseverance unlock  
  #define LIGHT_THRESHOLD 30                        // reading required to unlock
  #define MAG_THRESHOLD 0                           // reading required to unlock
  #define DISTANCE_THRESHOLD 16                     // reading required to unlock
  #define INACTIVITY_TIME   1200                    // seconds to wait before issuing inactivity taunt  
...
void setup() {
  ...
  actServo.write(ACTUATOR_HOME_POS);      // start with actuator in home position
...
void loop() {
  ...
  lcd.noBlink();
  bSwitchState = digitalRead(SWITCH_PIN);             // Step 1 - get the switch reading
  digitalWrite(LED_PIN, bSwitchState);                // Step 2 - set LED according to switch reading (switch on turns LED on)
  ...
  if(bSwitchState == 1)                                 // Step 3 - if switch is on then process behaviors
  { 
    iprevSwitchState = 1;            
    ...
    if(itoggleCount == 17) 
    {
      ...
      if(checkLockStatus() == LOCK_STATUS_UNLOCKED)
      {
        ...
        {
          displayLCD(F("Amazon gift"),F("card follows:"), 60,5000);       
          displayLCD(F("0000-0000-0000"),F("0000-0000-0000"), 0,10000);
        }
        else
        { 
          displayLCD(F(" *** Locked *** "),F(""), 0,10000);        
        }
      }
    }
    else if (itoggleCount > 12)
    {
    ...
    else if (itoggleCount == 8)
    {
      ...
      {
        displayLCD(F("Hint must unlock"),F("all 4 readings"), 0,2000);         
      }
      else
      {
    ...
    else if(itoggleCount > 1) flap(itoggleCount);
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,101) <= SPEAK_TO_ME_PROB)) speak();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,101) <= MAGNETO_PROB)) DisplayMagReading();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,101) <= LIGHT_PROB)) iDisplayLightReading();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,101) <= DISTANCE_PROB)) iDisplayDistanceReading();
    ...
    *********************************************************/
    bSwitchState = bcheckSwitchState();  
    if(bSwitchState == 1 && (myrandom(1,101) <= PERSEVERANCE_PROB)) iDisplayPerseverance();
    ...
    *********************************************************/
    ...
    else
    {
      ...
      if((myrandom(1,101) <= GIVEUP_PROB) || iPerseverance < 10) 
      {
...
{
  int iret = 0;
  // adapted from: http://forum.arduino.cc/index.php?topic=3129.0
  iret = min + random() % (max - min);
