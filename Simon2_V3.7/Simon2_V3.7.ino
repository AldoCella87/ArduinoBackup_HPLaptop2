 ...
 * only display global statistics when changing levels
 */
...
void setup() {
  ...
  // detect request for setup mode
  if(digitalRead(buttonPins[SETUP_ENTRY_KEY]) == HIGH){
...
void loop() {
  ...
  if (bDisplayNextChallenge){
    ...
    // initialize button response matrix for upcoming round
...
/*********************************************************************
 * setup next challenge and display it.  
