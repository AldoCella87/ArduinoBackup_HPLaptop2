 ...
 * quick fix is to ensure all lights are off at start of challenge display.  fixed: 4/8/21 - set all LEDs off when timout occurrs, 
 * user could be pressing a button when timout occurrs so light should be extinguished
 * 
...
// 4/8/21 - set all LEDs off when timout occurrs, user could be pressing a button when timout occurrs so light should be extinguished
...
#define xEST_MODE
    ...
    #define SPECIAL_GREETING1_TIME  99999   // round number to play special greeting                   
#else
...
void DisplayGlobalStats(void){
  ...
  // calculate percentages - this approach subject to dropping unrounded portion,
  // alternative is if there are timeouts then simply take 100 - (total of error and success) and assign that value, otherwise
  // there may be a fractional portion that is not reported.
  iErrorPct = (int)(fErrorPct * 100) + 0.5;       // add 0.5 to round up if needed
  iSuccessPct = (int)(fSuccessPct * 100) + 0.5;   // add 0.5 to round up if needed
  iTimeoutPct = (int)(fTimeoutPct * 100) + 0.5 ;  // add 0.5 to round up if needed  
